...
import com.iamazy.elasticsearch.dsl.sql.model.ElasticSqlQueryFields;
import com.iamazy.elasticsearch.dsl.sql.model.QueryFieldReferenceNode;
import com.iamazy.elasticsearch.dsl.sql.model.QueryFieldReferencePath;
...
public class QueryFieldParser {
    ...
    private static final String DOUBLE_NESTED_DOC_IDF = "$$";
    ...
    private static List<QueryFieldReferenceNode> parseQueryFieldExprToRefPath(SQLExpr queryFieldExpr) {
        ...
        if (queryFieldExpr instanceof SQLPropertyExpr) {
            ...
            boolean checkSyntax=!expr.contains(NESTED_DOC_IDF)
                    ...
                    ||(StringUtils.countMatches(expr,NESTED_DOC_IDF)==3 &&StringUtils.countMatches(expr,DOUBLE_NESTED_DOC_IDF)==1&&expr.indexOf(NESTED_DOC_IDF)!=expr.indexOf(DOUBLE_NESTED_DOC_IDF));
            if(!checkSyntax){
            ...
            for (String strRefNode : queryFieldTextList) {
                if(strRefNode.contains(DOUBLE_NESTED_DOC_IDF)&&strRefNode.indexOf(NESTED_DOC_IDF)!=strRefNode.indexOf(DOUBLE_NESTED_DOC_IDF)){
                    Integer doubleDollarIdx=strRefNode.indexOf(DOUBLE_NESTED_DOC_IDF);
                    String[] split = new String[]{strRefNode.substring(0,doubleDollarIdx),strRefNode.substring(doubleDollarIdx+2)};
    ...
    private static QueryFieldReferenceNode buildReferenceNode(String strRefNodeName) {
        ...
        if (strRefNodeName.contains(NESTED_DOC_IDF)) {
            ...
            } else if (StringUtils.countMatches(strRefNodeName, DOUBLE_NESTED_DOC_IDF) == 1 && StringUtils.countMatches(strRefNodeName, NESTED_DOC_IDF) == 2) {
                referenceNode = new QueryFieldReferenceNode(strRefNodeName.replace(DOUBLE_NESTED_DOC_IDF, CoreConstants.DOT), true);
            } else if (StringUtils.countMatches(strRefNodeName, NESTED_DOC_IDF) == 3 && StringUtils.countMatches(strRefNodeName, DOUBLE_NESTED_DOC_IDF) == 1 && strRefNodeName.indexOf(NESTED_DOC_IDF) != strRefNodeName.indexOf(DOUBLE_NESTED_DOC_IDF)) {
                if (strRefNodeName.startsWith(NESTED_DOC_IDF)) {
                    referenceNode = new QueryFieldReferenceNode(strRefNodeName.substring(1).replace(DOUBLE_NESTED_DOC_IDF, CoreConstants.DOT), true);
                } else {
                    referenceNode = new QueryFieldReferenceNode(strRefNodeName.replace(DOUBLE_NESTED_DOC_IDF, CoreConstants.DOT).replace(NESTED_DOC_IDF, CoreConstants.DOT), true);
                }
            } else {
                throw new ElasticSql2DslException("[syntax error] nested doc query can not support this syntax");
            }
