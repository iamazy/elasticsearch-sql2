...
import com.iamazy.springcloud.elasticsearch.dsl.sql.model.ElasticSqlQueryFields;
import com.iamazy.springcloud.elasticsearch.dsl.sql.model.QueryFieldReferenceNode;
import com.iamazy.springcloud.elasticsearch.dsl.sql.model.QueryFieldReferencePath;
    ...
    private static QueryFieldReferenceNode buildReferenceNode(String strRefNodeName) {
        ...
        if (strRefNodeName.contains(NESTED_DOC_IDF)) {
            ...
            if(strRefNodeName.startsWith(NESTED_DOC_IDF)&&strRefNodeName.lastIndexOf(NESTED_DOC_IDF)==0) {
                ...
                referenceNode = new QueryFieldReferenceNode(strRefNodeName.substring(1).replaceAll("\\$", "\\."), true);
            }
            else {
                referenceNode = new QueryFieldReferenceNode(strRefNodeName.replaceAll("\\$", "\\."), true);
            }
        } else {
